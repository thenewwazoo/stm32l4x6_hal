...
impl Constrain<Rcc> for RCC {
    ...
    ///
    /// Per 6.2, "The MSI is used as system clock source after startup from Reset, configured
    /// at 4 MHz."
    fn constrain(self) -> Rcc {
        // Enable write access to the BDCR
        unsafe { (*PWR::ptr()).cr1.modify(|_,w| w.dbp().set_bit()); }
...
pub mod clocking {
    ...
    impl RtcClkSource {
        pub fn freq(&self, hse: Option<HighSpeedExternalOSC>) -> Result<u32, ()> {
            match *self {
                RtcClkSource::None => Err(()),
                RtcClkSource::LSI => Ok(32_000),
                RtcClkSource::LSE => Ok(32_768),
...
impl CFGR {
    ...
    pub fn freeze(self, acr: &mut ACR) -> Clocks {
        ...
        let (sys_clock, sw_bits) = match self.sysclk {
            clocking::SysClkSource::MSI(s) => {
                s.configure(rcc)
            },
            clocking::SysClkSource::HSI16(s) => {
                s.configure(rcc)
            },
            clocking::SysClkSource::HSE(s) => {
                s.configure(rcc)
            },
            clocking::SysClkSource::PLL(s) => {
                s.configure(rcc)
            },
        };
        ...
        unsafe { (*PWR::ptr()).cr1.modify(|_,w| w.dbp().clear_bit()); }
